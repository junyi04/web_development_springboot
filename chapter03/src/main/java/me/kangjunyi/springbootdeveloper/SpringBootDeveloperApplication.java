...
/*
    ...
    test/java 폴더에 JUnitTest.java 폴더 생성

    AssertJ  로 검증문 가독성 높이기

        AssertJ 는 JUnit 과 함계 사용해 검증문의 가독성을 높여주는 라이브러리
        예를 들어서 아까 작성한 JUnitTest.java 에서
        Assertions.assertEquals(sum, a + b); 를 보시면 기대값과 비교값이 잘 구분되지 않습니다.

        AssertJ 를 사용한 코드 라인
        assertThat(a+b).isEqualTo(sum)
        이 경우 a와 b를 더한 값이 sum 과 같아야 한다는 의미로 읽힘 -> 영어의 경우
        
        자주 사용하는 메섣 목록
        
        1) isEqualTo(A) : A 와 같은지 검증
        2) isNotEqualTo(A) : A 와 다른지 검증
        3) contains(A) : A 를 포함하는지 검증
        4) doesNotContain(A) : A 를 포함하지 않는지 검증
        5) startsWith(A) : 접두사가 A인지 검증
        6) endsWith(A) : 접미사가 A인지 검증
        7) isEmpty() : 비어있는 값인지 검증
        8) isNotEmpty() : 비어있지 않은지 검증
        9) isPositive() : 양주인지 검증
        10) isNegative() : 음수인지 검증
        11) isGreaterThan(1) : 1보다 큰지 검증
        12) isLessThan(1) :
        13) isNotNull() :
        14) isNull() :
 */
