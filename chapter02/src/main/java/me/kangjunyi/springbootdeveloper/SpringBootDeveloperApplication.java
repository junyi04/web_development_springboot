...
/*
    1. 스프링 부트 3 구조 살펴보기
        ...
        계층 : 각자의 역할과 책임이 있는 어떤 소프트웨어의 구성 요소
            -> 각 계층은 호촐할 수 있지만 다른 계층에 직접 간섭하거나 영향을 미치진 않음

        ex) 카페와 빵집
            카페에서는 커피를 팔고, 빵집에서는 빵을 파는데, 필요한 경우 협업 관계를 맺어
        ...
        스프링 부트에서의 계층
            ...
            2) 비지니스 (Business)
                모든 비지니스 로직을 처리
                    * 비지니스 로직 : 서비스를 만들기 위한 로직을 의미하며, 웹 사이트에서 벌어지는 모든 작업
                        -> 주문 서비스라고 한다면, 주문 개수, 가격 등의 데이터를 처리하기 위한 로직,
                        주문 처리를 하다가 방생하는 예외 처리 로직, 주문을 받거나 취소하는 등
                        프로세스를 구현하기 위한 로직 등을 의미
                서비스가 비지니스 계층의 역할을 함

            3) 퍼시스턴스 (Persistence)
                모든 데이터 베이스 관련 로직을 처리. 이 과정에서 데이터 베이스에 접근하는
                DAD 객체를 사용할 수도 있음.
                    * DAD - 데이터베이스 계층과 상호작용하기 위한 객체라고 이해하면 됨
                    -> Repository 가 퍼스스턴스 계층의 역할을 함

        계층은 개념의 영역이고 Controller, Services, Repository 는 실제 구현을 위한 영역

    스프링 부트 프로젝트 디렉토리 구성
        : 스프링 부트에는 정해진 구조가 없지만, 추천 프로젝으 구조가 있고, 이를 많은 개발자들이 따름.
            수업 시에도 해당 구조를 따라 개발할 예정.

            main : 실제 코드를 작성하는 공간. 프로젝트 실행에 필요한 소스 코드나 리소스 파일은
                모두 이 폴더 안에 들어이끔 (프로젝트 생성 시 자동 구현됨)
            ...
            지시 사항
                main 디렉토리 구성하기
                    : main 디렉토리 내에 java 와 resources 로 구성돼 있음. 여기에 아직 추가ㅏ지 못했던
                        스프링 부트 프로젝트의 구성 요소를 추가
                    01 단계 - html 과 같은 뷰 관련 파일 (chapter07 수업 예정)을 넣을 templates 디렉토리 생성
                    ...
                    03 단계 - 스프링 부트 설정을 할 수 있는 application.yml 파일을 생성
                        -> 해당 파일은 스트링 부트 서버가 실행되면 자동으로 로딩되는 파일로,
                        데이터베이스의 설정 저보, 로깅 설정 정보 들이 들어갈 수도 있고, 직접 정의할 때
                        사용하기도 함. 앞으로 프로젝트를 진행하며 자주 사용할 파일
                        resources 우클릭 -> new file -> application.yml

    2. 스프링 부트 3 프로젝트 발전시키기

        각 계층의 코드를 추가할 예정 -> 이를 통해 계층이 무엇이고, 스프링 부트에서는 계층을 어떻게
            나누는지를 잡아나갈 수 있도록 하겠음.
        ...
        아직은 의존성이 무슨 역할을 하는지 몰라도 됨.

        implementation : 프로젝트 코드가 컴파일 시점과 모두 해당 라이브러리를 필요로 할 때
        ...
        compileOnly : 컴파일 시에만 필요, 런타임에는 포함되지 않아야하는 의존성 지정
        annotationProcessor : 컴파일 시에 애너테이션을 처리할 때 사용하는 도구의 의존성 지정

    지시 사항
        프레젠테이션, 서비스, 퍼시스턴스 계층 만들기

            1. 프레젠테이션 계층에 속하는 컨트롤러 관련 코드를 작성 -> TestController 가 있으므로 생량
            2. 비지니스 계층 코드 -> TestController 같은 위치에 TestService.java 생성
            3. 퍼시스턴스 계층 작성 -> 같은 위치에 Member.java 생성 -> 실제 DB에 접근하는 코드 작성
            4. 매핑 작업에는 '인터페이스' 파일이 필요. MemberRepository 인터페이스를 같은 위치에 생성

        작동을 확인

            01 단계 - resources 폴더에 sql문 하나 추가
                resources 우클릭 -> new -> file -> data.sql
            02 단계 - 기존에 만들어둔 application.yml 파일을 수정
