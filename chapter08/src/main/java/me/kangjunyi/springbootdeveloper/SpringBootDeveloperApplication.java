...
/*
    학습 목표 : 로그인 / 회원 가입 기능을 구현
        ...
        -> UserViewController 가 해당 요청에 대한 분기 처리를 하고 WebSecurityConfig 에 설정한 보안 관련
            내용들을 실행
        -> UserDetailsService 가 실행하면 요청을 성공했을 때,
        ...
        2) 스프링 시큐리티
            : 스프링 기반 애플리케이션의 보안을 담당하는 스프링 하위 프레임워크 보안 관련 옵션을 다수
                제공해줌.
                CSRF, 세샨 공격을 방어해주고(-> 정처기 등에 자주 나옴), 요청 헤더도 보안 처리를 해주므로
        ...
        3) 필터 기반으로 동작하는 스프링 시큐리티
            ...
            모두 외워야 하는거 x, 다만 로그린 과정이 어떤 흐름으로 이어지는 지를 이해하면 더 잘 활용 가능
            일단은 회원 가입 / 로그인 관련 부분을 코딩으로 풀어나갈 예정


    2. 회원 도메인 만들기
        스프링 시큐리티를 활용한 인증, 인가 가능 구현 예쩡
        회원 정보를 저장할 테이블을 만들고,
        테이블과 연결할 도메인을 만들고,
        이 테이블과 연결할 회원 엔티티를 만들겁니다.
