...
/*
    이메일로 사용자를 식별 가능, 즉 email 을 username 으로 봐도 됩니다.
    스프링 시큐리티가 이메일을 전달받아야 합니다. 스프링 데이터 JPA 는 메서드 규칙에 맞춰서 선언하면
    이름을 분석해 자동으로 쿼리(SQL)을 생성해줍니다.

    findByEmail() 메서드는 실제 데이터베이스에 회원 저옵을 요청할 때 다음 쿼리를 실행합니다.

    😘 findBtEmail() 메서드가 요청하는 쿼리의 예시
