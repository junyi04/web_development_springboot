...
/*
    ...
    블로그 글 작성을 위한 API 구현하기
        ...
        서비스 메서드 코드 작성
            ...
            컨트롤에서 여청한 본문을 받을 객체인 AddArticleRequest.java 파일을 생성

            DTO(Data Transfer Object) : 계층끼리 데이터를 교환하기 위해 사용하는 객체
            DTO는 데이터베이스와 연결되고 데이터를 조회하고 수정하는 사용되는 객체라 비교가 필요합니다.
            DAO의 경우에는 데이터 수정 관랸된 로직이 포함되지만 DTO는 단순하게 데이터를 옮기기 위해 사용되는 정달자 역할
            -> 그래서 병돌의 비지니스 로직이 필요하지 않다.
