...
import me.kangjunyi.springbootdeveloper.repository.BlogRepository;
...
public class BlogService {
    ...
    /*
        @Service : 해당 클래스를 빈으로 서블릭 컨테이너에 등록해줍니다.
        ...
        -> 컨트롤러 메서드 코드 작성하기
            URL에 매핑하기 위한 컨트롤러 메서그를 추가 -> 컨트롤러 메서그 구현 자체는 한 적 있는데,
            ...
            /api/article에 POST 요청이 들어오면 @PostMapping을 이용해 요청을 매핑한 뒤,
    ...
    /*
        ...
        이제 BlogApiController.java 이동하겠습니다.
     */

    // id로 특정 글 조회
    public Article findById(long id) {  // 이 경우 결과값은 하나밖에 없기 때문에 리턴 타입이 Article이여야만 합니다.
    ...
    public Article update(long id, UpdateArticleRequest request) {
        ...
        // domain 패키지의 Article 에서 정의한 update() 메서드 활용
        article.update(request.getTitle(), request.getContent());

        return article;
    }
    /*
        트랜잭션 - 데이터베이스에서 데이터를 바꾸기 위한 작업 단위

            예를 들어 계좌 이체 할 때 이런 과정을 거진다고 가정합니다.
                1) A 계좌에서 출금
                2) B 계좌에 입금
            그런데 1)은 성곻했는데, 도중에 2)가 실패하면 어떻게 되나요? 고객 입장에서는 출금은
            됐는데, 입금이 안된 상황이 발생하빈다.
            이런 상황이 발생하지 않으려면 출금과 입금을 하나의 단위로 묶어서,
            즉, 트랜잭션이라는 단위로 묶어서 두 작업을 한 단위로 실행하면 됩니다.
            만약 중간에 실패하면 트랜잭션의 처음 상태를 모두 되돌리면 됩니다 (ROLLBACK)
     */
}
