...
/*
    Given
        ...
        DB에 저장, 토큰 생성 API 의 요청 본문에 토큰을 포함하여 요청 객체를 생성

    When
        - 토큰 추가 API 에 요청을 보냄. 이때 요청 타입은 JSON,
        given 절에서 미리 만들어둔 객체를 요청 본문으로 함께 보냄.
...
class TokenApiControllerTest {
    ...
    // chapter06 - BlogApiControllerTest 참고
    @Autowired
...
/*
    chapter 10
        토큰 기반 인증의 특징
        그 중에서 JWT 를 적용하고, 토큰 제공자를 만듦.
        
        해당 인증 방법은 대중적으로 사용되고
        JWT 이 좀 현재 상황에서는 인기가 있는 편
        ...
        JWT : 토큰 기반 인증 중 현재 인기 있는 방식 JSON 형식으로
            ...
            구성 : 
                헤더 / 내용 / 서명 구조로 이루어져있음.
                헤더 - 토큰의 타입과 해싱 알고리즘 (JWT / HS256 적용)
                내용 - 토큰에 담을 정보
                서명 - 토큰이 조작되었거나 변경되지 않았음을 확인하는 용도

            Refresh Token : 액세스 토큰과 별개의 토큰. 액세스 토큰이 만료되었을 때
                새로운 액세스 토큰을 발급받는 용도
                (CreateAccessTokenRequest 로 RefreshToken 을 포함해서 요청했고, 그
                결과값으로 AccessToken 을 반환받는 구조로 구현했었음.)

            Filter : 실제로 요청되기 전과 후에 URL 패턴에 맞는 모든 요청을 처리하는 기능 제공
                - 세션 로그인 기준으로는 API 하나하나마다 요청 전후에 검증이 필요했었음.
                
            Security Context : 인증 객체가 저장되는 보관소, 인증 정보가 필요할 때마다
                인증 객체를 꺼내어 사용하도록 제공되는 클래스.
                시큐리티 컨텍스트 객체를 저장하는 객체 -> Security Context Holder
 */