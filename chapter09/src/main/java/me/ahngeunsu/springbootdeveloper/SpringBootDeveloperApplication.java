    ...
    사용자가 서버에 접근할 때 이 사용자가 인증된 사용자인지 확인하는 방법 중 하나로,
        ...
        토큰 기반 인증 :
            *토큰 : 서버에서 클라이언트를 구분하기 위한 유일한 값
            ...
            서버가 토큰을 클라이언트에게 제공하면, 클라이언트는 이 토큰을 가지고 ㅇ;ㅣㅆ다가
            여러 요청을 해당 토큰과 함께 신청함. 이후 서버는 토큰을 확인하며 유효한 사용자인지를
            검증하게 됨.
            ...
            토큰을 전달하고 인증 받는 과저
                ...
                2) 서버는 아이디와 비밀번호를 확인, 유요한 사용자라면 토큰을 생성하면서 응답 ->
            ...
            토큰 기반 인증의 특징
                ...
                2) 확장성
                    무상태성과 관련하여, 서버를 확장할 때 상태 관리를 신경 쓸 필요가 x
                    세션 기반 인증은 각각 API 에서 인증을 해야하는 것과 달리 토큰 기반 인증은
                ...
                3) 무결성
                    톹큰 기반 인증 방식은 HMAC(Hash-based Message Authentication) 기법이라고도 하는데,
                    토큰을 발급한 이후에는 토큰 정보를 변경하는 행위를 할 수 없음.
                    만약 토큰을 한 글자라도 변경하면 서버에서는 유효하지 않은 토큰이라고 판단
                    
                JWT (JSON Web Token) -> JSON 형태로 주고 받음
                    발급받은 JWT 를 이용해 인증을 하려면 HTTP 요청을 헤더 중에 Authorization 키 값에
                    Bearer + JWT 토큰 값을 넣어 보내야 함.

                    JWT 구조 : 온점(.)을 기준으로 헤더(header), 내용(payload), 서명(signature)로 여겨짐.
                        aaaa . bbbb . cccc
                         헤더    내용   서명
                        헤더에는 토큰의 타입과 해싱 알고리즘을 지정하는 정보를 포함함.
                        ...
                        2) 내용에는 토큰과 관련된 정보를 담습니다. 내용의 한 덩어리를 클레임(claim)
                            ...
                            구성 :
                                ...
                                exp : 토큰의 만료 시간 (expiration).
                                    시간은 NumericDate 형식 (20250113),
                                    항상 현재 시간 이후로 설정해야 합니다.
                                nbf : 토큰의 활성 날짜와 비슷한 개념. Hot Before 를 의미함.
                                    이 날짜가 지나기 전까지는 토큰이 처리되지 않음.
                                iat : 토큰이 발급된 시간. issued at 을 의미.
                                jti : 토큰의 고유 식별자로서 주로 일회용 토큰에 사용

                            공개 클레임 : 공개되어도 상관없는 클레임.
                                충돌을 방지할 수 있는 이름을 가져야 하며 보톤 URI
                            비공개 클레임 : 공개되면 안되는 클레임.
                                클라이언트와 서버 간의 통신에 사용

                            😘 JWT 예시
                            {
                                "iss" : "amber542825@gmail.com" // 등록된 클레임
                                "iat" : 12345678
                                "exp" : 12345678
                                "https://ahngeunsu.com/jwt_claims/is_admin" : true

                                iss, iat, exp 는 JET 자체에서 등록된 클레임
                                URI 로 네이밍 된 "https://ahngeunsu.com/jwt_claims/is_admin" : 공개클레임
                                등록 / 공개 클레임 제외 나머지는 전부 비공개 클레임에 해당"

                        3) 서명 : 해당 토큰이 조작되었거나 변경되지 않았음을 확인하는 용도로 사용.
                            헤더와 인코딩 값과 내용의 인코딩 값을 합친 후에 비밀 키를 사용해 해시값을 생성

                            토큰 유효 기간
                                토큰 유효 기간이 하루라면 하루동안 모든 인증을 할 수 있고,
                                토큰 유효 기강이 너무 짧다면 사용자가 불편함.
                                    -> 리프레시 토큰의 개념이 발생
        ...
        2. JWT 서비스 구현
            ...
            -> JWT 서비스를 사용할 준비가 됐다고 볼 수 있음.
            build.gradle 로 넘어가기
            
            토큰 제공자를 추가
                jwt 를 사용해서 JWT 를 생성하고 유효한 토큰인지 검증하는 클래스 추가 예정
                    JWT 토큰을 만들려면 이슈 발급자 (issuer), 비밀키 (secreat_key) 를 필수로 설정
