...
--	JOIN의 경우에는 여러가지 조건을 미리 충족시켜줘야만 합니다. -> ON
...
(SELECT id, phone, city, county 			-- 일부 컬럼만 지정해서 출력
	FROM users)
UNION ALL
(SELECT id, phone, city, country			-- 기준이 되는 첫번째 select절에서 선택하는
	FROM users)								-- 컬럼의 종류 및 개수가 완벽히 일치해야합니다.
	ORDER BY id
	;

-- 실무에서는 UNION ALL이 더 권장됩니다. UNION의 경우 중복 제거가 있는데, 다량의 데이터를 대상으로
-- 중복 제거할 때 컴퓨터에 무리한 연산 부하는 줄 가능성이 있기 때문에
-- 일단 UNION ALL을 통해서 최종 결과 형태 확인 후에 -> UNION을 적용하는 식으로
-- 프로세스가 짜여있는 편

-- users에서 country가 Korea인 회원 정보만 추출하고(1번 추출),
-- Mexico인 회원 정보만 추출해서(2번 추출) 결합해보자.
-- (단, 컬럼은 id, phone, city, country만 출력하고, 회종 결과 집합은 country 기준 알파벳 순)
...
-- 그 다음 순간에 "0" != "1"이 다른 시점에 들어갔을 때 크기 비교가 이뤄집니다.

-- YYYY-MM-DD 형식으로 저장돼있다면, 문자열 비교 결과와 실제 날짜 비교 결과가 동일하게 적용함.
...
-- 사용해 하나로 결합하라(단, 최종 결과는 id, phoce, country, city, is_marketing_agree
...
-- 서브쿼리
-- SQL 쿼리 결과를 테이블터럼 사용하는, 쿼리 속의 쿼리
...
;
-- SELECT AVG(price) FROM products;를 하는 경우 전체 price / 행의 개수로 나눈 대이터가
-- 단 하나이므로 SELECT name, price, AVG(price) FROM products; 로 작성하면
-- 1행짜리만 도출됨.

-- 이를 막기 위해서 서브쿼리를 적용했습니다.

-- products 테이블의 name / price를 불러오는 것은 기본적은 select절입니다.
...
-- 서브 뭐리를 작성하는 편이 권장됩니다.

-- 스칼라 거브 쿼리를 작성할 때 '단일 값'이 반환되도록 작성해야한다는 점에 유의하세요.
-- 만약 2개 이상의 집계 값을 기존 테이블에 추가하여 출력하고 싶다면 스칼라 서브 쿼리로 따로
-- 나누어서 작성해야 합니다.
...
-- 2번 쿼리가 서브 쿼리 포함된 해설 -> 해당 문제에서 눈여겨 볼 것은 굳이 따지자먄
-- 여기서는 서브쿼리를 작성하는데 있어서 스칼라 서브 쿼리 형태로 작성하는 것이 중요하다.
-- 해당 문제에서는 검증 결과 서브 쿼리 자체가 필요하지 않습니다.
...
-- 이상의 코드는 staff 케이블에서 id 값이 3, 5를 도출해냈습니다.
...
-- 이상의 코드에서 서브 쿼리의 결과 도출되는 컬럼의 숫자 = 1 (staff 테이블의 id) / 행 = 2

SELECT *
	FROM orders
	WHERE (staff_id, user_id) IN (			-- 필터링 대산 컬럼 개수 = 2
...
-- 결과값으로 직원 정보 테이블에 존재하는 id, user_id와 동일한 orders 테이블의
-- staff_id, user_id 컬럼에 있을 경우 변환하여 출력합니다.
-- 이상의 쿼리문 해석 -> 직원 자신이 자기 쇼핑몰에서 주문한 이력이 반환된 것.

-- products에서 discount_price가 가장 비싼 제품 정보를 출력하세요.
-- (단, products의 전체 컬럼이 다 출력)
...
SELECT *
	FROM (
		SELECT *
			FROM orders
			WHERE order_date >= "2015-07-01"
				AND order_date < "2015-08-01") o		-- 1)의 경과가 테이블이었기 때문에 별칭 o를 줌
		INNER JOIN (
			SELECT *
				FROM orderdetails 
...
-- 여러 개의 컬럼을 투가하고 싶을 때는 서브 쿼리를 여러 개 작성하면 됩니다.
-- 특징 : SELECT의 서브 쿼리는 메인 뭐리의 FROM에서 사용된 테이블이 아닌 테이블도 사용이 가능하기
-- 때문에 불필요한 조건 수행을 줄일 수 있다는 장점이 있다.
...
-- 이 때 조인 연산을 위해 별칭을 생성 가능한데 서브 쿼리가 끝나는 괄호 뒤에 공백을 한 칸 주고
-- 원하는 별칭을 쓰면 된다 (orders o / orderdetails od와 같은 방식)

-- 특징 :
-- FROM 에서 서브쿼리를 적절히 활용하면 적은 연산으로 같은 결과를 도출 가능. 단, RDBS 기준
-- 테이블 검색을 빠르게 할 수 있는 인덱스* 개념이 있는데 서브 쿼리를 활용하면 인덱스를 사용하지 못하는
-- 경우가 있으므로 주의해야 함. 

-- 인덱스(index) : 테이블의 검색 속도를 높이는 기능으로, 컬럼 값을 정렬하며 검색 시 더욱 빠르게
-- 찾아내도록 하는 자료구조
...
-- 위의 예세에서는 IN 연산자를 사용했지만, 다른 비교 연산자도 사용 가능.

-- 특징 : IN 연산자의 경우에 다중 컬럼 비교를 할 때는 서브 쿼리에서 추출하는 컬럼의 개수와
-- WHERE에 작성하는 필터링 대상 컬럼 개수가 일치해야 함. -> 이 때 필터링 대상 컬럼이 2개 이상이면
-- ()로 묶여서 작성해야 합니다.

-- 1. 데이터 그룹화하기 (GROUP BY + 집계 함수)
-- 2. 데이타 결과 집합 결합하기 (JOIN + 서브쿼리)
-- 3. 테이블 결합 후 그룹화하기 (JOIN + GROUP BY)
-- 4. 서브 쿼리로 필터링 (WHERE절 + 서브 쿼리)
-- 5. 같은 행동 반복 대상 추출 (LEFT JOIN)

-- 1. users에서 created_at 컬럼 활용하여 연도별 회원 수를 추출
...
SELECT country, city, SUM(is_auth)		-- is_auth가 1이면 본인인증한거니까 is_auth들의 합이 10이라면
	FROM users							-- 본인인증한 회원 숫자가 10이라는 뜻이하서
