...
-- select절 - from절 - group by절 - order by절 : 순서

GROUP BY 에 두 개 이상의 기준 컬럼을 추가하면 데이터가 여러 그룹으로 나뉨
아르헨티나를 기준으로 했을 때 마케팅 수신 동의 여부가 0인 회원 수와 마케팅 수신 여부가 1인
회원 수를 기준으로 나뉘어져 있음을 알 수 있음.

예를 들어 위의 쿼리문의 경우, country를 기준으로 먼저 그룹화가 이루어지고
그 후에 is_marketing_agree를 기준으로 그룹화

그래서 GROUP BY 에 여러 기준을 설정하면, 컬럼 순서에 따라 결과가 달라짐
따라서 '중요한 기준을 앞에 배치'해서 시각화와 그룹화에 대한 우선순위를 결정할 필요가 있음.

-- GROUP BY 정리
-- 1) 주어진 컬럼을 기준으로 데이커를 그룹화하여 '집계 함수'와 함께 사용
-- 2) GROUP BY 뒤에는 그룹화할 컬럼명을 입력. 그룹화한 컬럼에 집계 함수 적용하여 그룹별 계산 수행
-- 3) 형식 : GROUP BY 컬럼명1, 컬럼명2, ...
-- 4) GROUP BY 에서 두 개 이상의 기준 컬럼을 조건으로 추가하여 여러 그룹으로 
-- 		분할 가능한데, 컬럼 순서에 따라 결과에 영향을 미치므로 우선순위를 생각할 필요 있음.

-- users에서 국가 내 도시별 회원수를 구하여 출력
...
-- 가입 일시 컬럼 활용하고, 최신순으로 정렬할 것.
SELECT SUBSTR(created_at, 1, 7) AS month, COUNT(DISTINCT id) AS uniaueUserCnt
	FROM users
	GROUP BY SUBSTR(created_at, 1, 7)
	ORDER BY SUBSTR(created_at, 1, 7) DESC;

-- 1. orderdetails에서 order_id 별 주문 수량 quantity의 총합을 출력할 것.
	-- 주문 수량의 총합이 내림차순으로 정려로디도록 할 것 (함수는 어제 수업에서 한 것 확인)
...
-- HAVING
-- group by 를 이용해서 데이터를 그룹화하고 해당 그룹별로 집계 연상을 수행하며,
-- 국가 별 회원 수를 도출해낼 수 있었음 (COUNT())
-- 예를 들어서 회원 수가 n명 이상인 국가의 회원 수만 보는 등의 조건을 걸려면 어떡해야할까?

-- WHERE절을 이용하는 방법이 있긴 하지만 추가적인 개념에 대해 학습하자
-- 언제나 WHERE을 쓰는 것이 용이하지 않다는 점부터 짚고 넘어가서 HAVING을 학습
...
WHERE을 통해서 원하는 국가만 먼저 필터링하고, GROUP BY 를 함
여기서 필터링 조건은 country 컬럼의 데이커 값에 해당

만약에 회원 수가 8명 이상인 국가의회원 수만 필터링 하려면?
...
SELECT country, COUNT(DISTINCT id)
	...
	GROUP BY country
	HAVING COUNT(DISTINCT id) >= 8
	ORDER BY 2 DESC					-- 두 번쨰 컬럼을 DESC순으로 정렬
	;

1. where에서 필터링을 시도할 때 오류가 발생하는 이유 :
	where은 GROUP BY 보다 먼저 실행되기 떄문에 그룹화를 진행하기 전에 행을 필터링 함.
	하지만 집계 함수로 계산된 값의 경우에는 그룹화 이후에 이루어지기 때문에
	순서상으로 GROUP BY 보다 뒤에 있어야 해서 WHERE 절 도입이 불가능 함.
	
2. 즉, GROUP BY 를 사용한 집게 값을 필터링 할 때는 -> HAVING 사용
...
-- HAVING 정리
-- 순서상 GROUP BY 뒤쫏에 위피하며, GROUP BY와 집계 함수로 그룹별로 집계한 값을
-- 필터링할 때 사용

-- WHERE과 동일하게 필터링 기능을 수행하지만, 적용 영역의 차이 때문에 주의할 필요가 있음.
-- WHERE은 'FROM에서 볼러온 데이터'를 직접 필터링하는 반면에,
-- HAVING은 'GROUP BY가 실행된 집계 함수 값'을 필터링 함.
-- 따라서 HAVING은 GROUP BY가 SELECT문 내에 없다면 사용할 수 없음.
...
연습 문제
1. orders에서 회원별 주문 건수를 추출하라 (단, 주문 건수가 7건 이상인 회원의 정보만 추출,
