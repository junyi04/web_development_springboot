...
-- 뿐만 아니라 특정 컬럼의 값이 A인 데이터만 가져오는 등 복잡한 쿼리는
-- 작성하는 일이 있음
...
-- 거주 국가가 한국이 아닌 회원만 추출하는 쿼리문을 작성하세요.
SELECT * FROM users WHERE country != "Korea";
...
-- between : 시작값과 종료 값을 포함하는 범위 내의 데이터를 조회
...
SELECT *
	FROM users
	WHERE created_at >= "2010-12-10" AND created_at <= "2011-01-01";
...
-- SQL을 해석하는 컴퓨터가 LIKE 코드응 읽는 순간 와일드 카드를 감지하는데
-- SQL에서의 와일드 카드를 '%'로 0개 이상의 임의의 문자열을 의미하는
...
-- IS : A IS B -> A는 B이다라는 뜻이기 때문에
-- A 컬럼의 값이 B이다 일 때 큭히 null 일 때 '=' 대신 사용
...
-- 연습 문제
	-- 1. users에서 country가 Mexico인 회원 정보만 추출.
		-- 단, created_at, phone, country 컬럼만 추출할 것
	SELECT created_at, phone, country 
		FROM users 
		WHERE country = "Mexico";
	-- 2. products에서 id가 20 이하이고 price는 30 이상인 제품 정보 출력
		-- 단, 기존 컬럼 전부 출력하고, 정상 가격에서 얼마나 할인 되었는지를
		-- (price - discount_price)
		-- discount_amount 라는 컬럼명으로 추출
	SELECT *,
...
-- 2. SELECT *,									-- 전부 다의 기존 컬럼
-- (price - discount) AS "discount_amount"		-- 새로운 컬럼
-- 5. NOT BETWEEN 순

-- ORDER BY
-- 현재까지 WHERE을 이용해서 특정한 조건에 합치하는 데이커들을 조회하는 SQL문에 대해서 학습

-- 이상의 경우 저장된 순서대로 정렬된 결과만 볼 수 있음 -> id 라는 PK 값에 따라

-- 이번에는 가져온 데이터를 원하는 순서대로 정렬하는 방법에 관한 것
...
-- 아스키 코드(ASCII Code)를 기준
	
-- 예제 : users에서 city를 기준으로 내림차순에 대해 전체 정보를 출력
SELECT *
	FROM users
	ORDER BY city DESC;
...
-- ORDER BY 특징 : 컬럼명 대신에 컬럼 순서를 기준으로 하여 정렬이 가능
	-- 컬럼명을 몰라도 정렬할 수 있다는 장점이 있지만 주의할 필요가 있음.

-- 예제 : users에서 username, phone, city, country, id 컬럼명을 순서대로 출력
...
-- 즉, ORDER BY는 데이터 추출이 끝난 후를 기점으로 적용된다는 점을 명심
