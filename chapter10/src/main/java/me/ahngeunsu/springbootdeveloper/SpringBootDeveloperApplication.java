...
/*
    사전 지식 : OAuth
         - 제 3의 서비스에 계정 관리를 맡기는 방식. naver/Google/kakao로 로그인 하기와 같은 예시
        ...
        OAuth 를 사용하면 인증 서버에서 발급받은 토큰을 사용해서 리소스 서버에
            리소스 오너의 정보를 요청하고 응답 받아 사용 가능.
            그렇다면 이소스 오너의 정보를 어떻게 취득할 수 있는지에 대한 방법이 필요,

            리소스 오너 정보를 취득하는 네 가지 방법
                1) 권한 부여 코드 승인 타입 (Authorization Code Grant Type) :
                    OAuth 2.0에서 가장 대중적인 인증 방법. 클라이언트가 리소스에 접근하는 데 사용.
                    권한에 접근할 수 있는 코드와 리소스 오너에 대란 액세스 토큰을 발급받는 방식.

                2) 암시적 승인 (Implicit Grant Type) :
                    서버가 없는 자바스크립 웹 애플리케이션 클라이언트에서 주로 사용.
                    클라이언트가 요청을 보내면 리소스 오너의 인증 과정이외에는 권한 코드 교환 등의
                    별다른 인증 과정을 거지치 않고 액세스 토큰을 제공 받는 형식
     ...
     1. 권한 요청    |       5. 액세스 토큰으로 데이터에 접근      | 인증 서버에
          ...
          1. 권한 요청 :
            ...
            즉 카카오나 구글 권한 서버에 요청을 보내는 거라고 볼 수 있음. 요청 URI 는 권한 서버마다 다르지만
            보톤은 클라이언트 ID, 리아이텍트 URI, 응답 파일 등을 파라미터로 보냄.
            ...
            client_id : 인증 서버가 클라이언트에 할당한 고유 식별자.
                이 값은 클라이언트 애플ㄹ리케이션 OAuth 서비스에 등록할 때 서비스에서 생성하는 값

            redirect_uri : 로그인 성공시 이동해야하는 URI
            response_type : 클라이언트가 제공받길 원하는 응답 타입. 인증 코드를 받을 때는 code 값을 포함해야 합니다.
        ...
        3. 인증 코드 제공
            사용자가 로그인에 성공하면 권한 요청 시에 파라미터로 보낸 redirect_uri 로 리다이렉션됩니다.
            이 때 파라미터에 인증 코드가 함께 제공.

            🎈 인증 코드 예시
            GET http://localhost8080/myapp?code=als2f3mcj2

        4. 액세스 코드 응답
            '인증 코드'를 받으면 '액세스 토큰'으로 교환해야 함.
            ...
            🎈 /token POST 요청 예시
        {
            ...
            "code": "a1b2c3d4e5f67h8"
                ...
                grant_type : 권한 유형을 확인하는 데 사용. 이 때는 authorization_code 로 설정해야 함. 권한 서버는 요청 값을 기반으로
        ...
        5. 액세스 토큰으로 API 응답 & 반환
            ...
            여기까지가 권한 부여 코드 승인 타입의 흐름입니다. 대부분의 OAuth 를 구현한 라이브러리는 이 흐름을 바탕으로 코드를 구현하기 때문에
    ...
    쿠키 관리 클래스를 구현
        ...
        springbootdeveloper 패키지 내에 util 패키지를 생성 -> CookieUtil.java 를 생성
